...
public class MELoggerScreen extends AEBaseScreen<MELoggerMenu> {
    ...
    private void drawWarningBanner(GuiGraphicsExtractor guiGraphics) {
        ...
        int y = top + 4;
        for (var line : lines) {
            guiGraphics.text(this.font, line, 4, y, WARNING_COLOR, false);
            y += lineHeight;
        }
    }

    private void drawWrapped(GuiGraphicsExtractor guiGraphics, Component text, int wrapWidth, int startY, int color) {
        int y = startY;
        for (var line : this.font.split(text, wrapWidth)) {
            guiGraphics.text(this.font, line, 8, y, color, false);
